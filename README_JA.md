...
- **スケジューラ統合**：RPMが枯渇した資格情報は選択時に自動的にスキップ
...
| `rpm_limit` | 資格情報attributes | string | — | 資格情報ごとのオーバーライド。`default-rpm`より優先 |

### 管理API

すべての資格情報の現在のRPMステータスを照会：

```bash
GET /v0/management/rpm-stats
```

各資格情報の現在のリクエスト数、設定された制限、リトライ待ち時間を返します。
