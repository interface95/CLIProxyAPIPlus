...
CLIProxyAPI Plusは、資格情報ごとのRPM（1分あたりのリクエスト数）レート制限をサポートしています。資格情報がRPM閾値を超えると、スケジューラは自動的にそれをスキップし、次に利用可能な資格情報を選択します。すべての資格情報がレート制限されている場合、プロキシは`Retry-After`ヘッダー付きのHTTP 429を返します。
...
```yaml
# グローバルデフォルト：資格情報ごとの1分あたり最大リクエスト数（0 = 無制限）
default-rpm: 60

# グローバルデフォルト：資格情報ごとの最大同時リクエスト数（0 = 無制限）
...
- **スライディングウィンドウアルゴリズム**：資格情報ごとに60秒ウィンドウ内のリクエストタイムスタンプを追跡
- **同時実行制御**：資格情報ごとの同時リクエスト数を制限（開始時にスロット取得、完了時に解放）
- **スケジューラ統合**：RPMが枯渇または同時実行数が上限の資格情報は選択時に自動的にスキップ
- **コンダクタ二重チェック**：実行前の二次チェックで並行処理の安全性を確保
- **自動回復**：リクエストがウィンドウから外れると資格情報は再び利用可能に
- **ゼロ設定デフォルト**：`rpm_limit=0`または未設定は無制限を意味（完全な後方互換性）

### 設定リファレンス

| 設定 | 場所 | 型 | デフォルト | 説明 |
|------|------|------|---------|------|
| `default-rpm` | `config.yaml` | int | `0` | すべての資格情報のグローバルデフォルトRPM制限。`0` = 無制限 |
| `rpm_limit` | 資格情報attributes | string | — | 資格情報ごとのRPMオーバーライド。`default-rpm`より優先 |
| `default-max-concurrent` | `config.yaml` | int | `0` | すべての資格情報のグローバルデフォルト最大同時実行数。`0` = 無制限 |
| `max_concurrent` | 資格情報attributes | string | — | 資格情報ごとの同時実行数オーバーライド。`default-max-concurrent`より優先 |
...
```bash
GET /v0/management/rpm-stats            # 資格情報ごとのRPMステータス
GET /v0/management/concurrency-stats    # 資格情報ごとの同時リクエスト数
```
